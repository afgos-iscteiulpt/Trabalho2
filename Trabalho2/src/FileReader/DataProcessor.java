...
public class DataProcessor {
	...
	public void processQuestion(String s) {
		...
		for (String word : stringArray) {
			switch (tabSplit[0].trim()) {
			case "actor_name" :
				actor_name_unigram.add(word);
				actor_name_bigram.put(lastRead, word, actor_name_bigram.get(lastRead, word));
				break;
			case "budget" :
	...
	public void countAllObjects() {
		Set<String> uniqueSet = actor_name_unigram.elementSet();
		for (String s : uniqueSet) {
			System.out.println(s + " -> " + actor_name_unigram.count(s));
		}
	}
